...
def tts_new_body(texts, voice, resource_id, rate, device, pitch="+0%"):
    babi = {
    ...
    for text in texts:
        voice_blocks.append(
            ...
            f'        <prosody rate="{rate}" pitch="{pitch}">{escape_xml(text)}</prosody>\n'
            f'    </voice>'
        )
    ssml = (
...
def build_request(args):
    ...
    if args.mode == "tts-new":
        ...
        babi, body = tts_new_body(texts, args.voice, args.resource_id, args.rate, device, getattr(args, 'pitch', '+0%'))
